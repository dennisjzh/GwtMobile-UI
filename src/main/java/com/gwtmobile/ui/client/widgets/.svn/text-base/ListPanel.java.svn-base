...
public class ListPanel extends PanelBase implements ClickHandler, DragEventsHandler {
	...
	public ListPanel() {
		...
		setStyleName(StyleNames.ListPanel);
	}
	...
	public void add(Widget w) {
		if (w.getClass().toString().endsWith("." + StyleNames.ListPanel)) {
			super.add(w);
		} else {
	...
	public void onClick(ClickEvent e) {
		if (_selected >= 0) {
			...
			if (!item.getDisabled()) {
				SelectionChangedEvent selectionChangedEvent = new SelectionChangedEvent(_selected, e.getNativeEvent()
						.getEventTarget());
				this.fireEvent(selectionChangedEvent);
				item.removeStyleName(SubStyleNames.pressed);
			}
			_selected = -1;
		}
	}
				...
				new Timer() {
					...
					public void run() {
						if (_selected >= 0) {
							ListItem item = (ListItem) getWidget(_selected);
							if (!item.getDisabled()) {
								getWidget(_selected).addStyleName(SubStyleNames.pressed);
							}
						}
					}
				}.schedule(75);
	...
	public void onDragMove(DragEvent e) {
		if (_selected >= 0) {
			getWidget(_selected).removeStyleName(SubStyleNames.pressed);
			_selected = -1;
		}
	}
	...
	static class Chevron extends HTML {
		public Chevron() {
			super("<div class=\"" + StyleNames.Chevron + "\"><span></span><span></span></div>");
		}
	}
