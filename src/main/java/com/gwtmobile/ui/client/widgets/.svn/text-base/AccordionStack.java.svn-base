...
public class AccordionStack extends PanelBase {
	...
	public void onInitialLoad() {
		if (this.getStyleName().indexOf(SubStyleNames.expand) == -1) {
			close();
		}
	}
	...
	public void close() {
		this.addStyleName(SubStyleNames.collapse);
		this.removeStyleName(SubStyleNames.expand);
	...
	public void expand() {
		...
		this.addStyleName(SubStyleNames.expand);
		this.removeStyleName(SubStyleNames.collapse);
		Scheduler.get().scheduleDeferred(new ScheduledCommand() {
			@Override
			public void execute() {
				Utils.console(_content.getElement().getScrollHeight() - _content.getElement().getOffsetHeight() + "px");
				_content.setHeight(_content.getElement().getScrollHeight() - _content.getElement().getOffsetHeight()
						+ "px");
			}
		});
	}
	...
	public boolean isExpended() {
		return this.getStyleName().indexOf(SubStyleNames.expand) > -1;
	}
